...
const ProductGrid = ({
  ...
  toggleCompare,
   ...
   const activeFilters = [
     ...
     filters.size,
     filters.shoeSize,
     filters.minPrice && `Min: PHP ${filters.minPrice}`,
     filters.maxPrice && `Max: PHP ${filters.maxPrice}`,
   ].filter(Boolean);

   const wishlistKeys = new Set(wishlistItems.map((item) => String(item.id ?? item.title)));
   const compareKeys = new Set(compareProducts.map((item) => String(item.id ?? item.title)));

   return (
     <div className="mx-auto max-w-[1440px] px-0 pt-8 pb-20">
       <div className="mb-8 flex flex-col gap-4 border border-black/5 bg-[#f7f7f4] p-4 md:flex-row md:items-center md:justify-between md:p-5 dark:border-white/10 dark:bg-zinc-900">
         <div className="flex flex-wrap gap-2">
           {activeFilters.length > 0 ? (
             activeFilters.map((filter) => (
               <span
                 key={filter}
                 className="border border-black/10 bg-white px-3 py-2 text-[10px] font-black uppercase tracking-widest text-gray-600 dark:border-white/10 dark:bg-zinc-950 dark:text-zinc-300"
               >
                 {filter}
               </span>
             ))
            ...
            {filteredAndSortedProducts.map((product, index) => (
              ...
              >
                <ProductCard
                  ...
                  isCompared={compareKeys.has(String(product.id ?? product.title))}
                />
              </motion.div>
            ))}
          </AnimatePresence>
        ...
        {!loading && !error && filteredAndSortedProducts.length === 0 && (
          ...
          >
            ...
            <button
              ...
              className="mt-6 border border-black px-5 py-3 text-[10px] font-black uppercase tracking-[0.2em] transition hover:bg-black hover:text-white dark:border-white/30 dark:hover:bg-white dark:hover:text-black"
            >
              Clear Filters
            </button>
          </motion.div>
